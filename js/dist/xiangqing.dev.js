...
$.ajax({
  ...
  success: function success(res) {
    $('.xq_tu img').attr('src', "".concat(res.data.pimg));
    $('.xq_tu1 img').attr('src', "".concat(res.data.pimg));
    $('#mingcheng').html("".concat(res.data.pname));
    $('#miaoshu').html("".concat(res.data.pdesc));
    $('#jiage span').html("".concat(res.data.pprice));
    shuzhi();
  }
}); //  加入购物车
...
var paid = location.search.split('=')[1];
console.log(btn);
...
window_drage(lens, box1, pic, 2); // 放大镜效果,box(模拟透镜),box1(存放需要放大的图片的盒子),pic(放大区域的图片),mul(放大倍数)
// 要给模拟透镜,放大区域的图片添加绝对定位

function window_drage(box, box1, pic, mul) {
  box1.onmouseenter = function (e0) {
    var evt0 = e0 || window.e0; // 在可视区域点击时，模拟透镜中心位置出现在鼠标点击处
    ...
    box.style.backgroundColor = "rgba(167, 188, 194,0.8)"; // 移动模拟透镜，放大区域图片按比例进行位移,并且将模拟透镜的唯一区域固定在box1中

    box1.onmousemove = function (e2) {
      var evt2 = e2 || window.e2;
      var l = evt2.clientX - box.offsetWidth / 2 - box1.offsetLeft;
      var t = evt2.clientY - box.offsetHeight / 2 - box1.offsetTop;
